...
The non-nullable types let you access all of the interesting methods, but can never ever contain `null`.
...
In simple words, `Never` means that the function never returns, e.g. it ends the process or always throws an exception. 
...
`Void` means that the function still returns, but doesn't return a value.
